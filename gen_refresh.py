...
TODAY = "2026-06-01"   # pinned: this refresh's content is dated 2026-06-01
NOW   = "15:51"        # pinned: preserve original generation stamp
...
body{background:var(--bg);color:var(--ink);font-family:var(--font);font-size:15px;line-height:1.65;overflow-x:hidden;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility}
::selection{background:var(--coral);color:#fff}
::-webkit-scrollbar{width:11px;height:11px}::-webkit-scrollbar-thumb{background:#cfc6b5;border-radius:6px;border:3px solid var(--bg)}::-webkit-scrollbar-track{background:transparent}
...
.regime-tag{display:inline-block;font-size:11px;font-weight:700;letter-spacing:.11em;text-transform:uppercase;color:var(--cream);background:var(--coral);border:1px solid var(--coral);border-radius:30px;padding:6px 15px;margin-bottom:1.3rem}
.gold-rule{display:none}
/* Panels (peach section boxes) + black banners */
.panel{background:var(--panel);border-radius:var(--radius-xl);padding:1.7rem 1.8rem}
.rhs .panel{padding:1.45rem 1.5rem}
.banner{background:var(--black);color:var(--cream);border-radius:var(--radius-xl);padding:2.1rem 2rem;transform:scale(.9);opacity:.5;transform-origin:center;will-change:transform,opacity;transition:box-shadow .4s ease}
...
# Peach panels everywhere; black scroll-expand banners for the two "feature" reads.
LHS = "\n".join([
    panel(YESTERDAY), banner(WRAP), panel(CORRELATION), panel(VOL_SKEW),
    panel(SECTOR_RV), panel(POSITIONING), panel(FUNDING), panel(TAPE_MISSING),
    panel(CONSENSUS), banner(ONE_CHART), banner(CAT_CAL), banner(EARNINGS),
    panel(MIND), panel(CLIENT_AMMO), panel(CITATIONS), panel(STALENESS),
])

RHS = "\n".join([
    panel('<div class="section-label">The Open</div>' + dashboard_html + theme_line),
    panel('<div class="section-label">New Trade Ideas</div>' + idea_cards),
...
(function(){
  ...
  function frame(){
    ...
    banners.forEach(function(el){
      ...
      var p=Math.max(0,Math.min(1,1-d));               // 1 when centred
